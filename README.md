...
The LLM has 3 different parts: generator, solver_model, and judge. The generator produces a novel question every iteration, the solver answers the question, and the judge evaluates it.
The utilization can be improved by omitting the generator part and giving the model one question, so the model should answer until the model reaches the 
